...
/**
 * IndiaMartSteps – Step Definitions for IndiaMart.feature.
 *
 * STRUCTURE:
 *   Each public method maps 1-to-1 to a Gherkin step.
 *   The method annotation string must EXACTLY match the feature file step text.
 *
 *   Page Objects are created lazily (only when the step that needs them runs)
 *   so we don't create unnecessary objects for scenarios that don't use them.
 *
 * DATA FLOW:
 *   TC01: names → step field → passed to Excel at end of scenario
 *         phones → populated by looping through detail page tabs
 *         ratings → extracted from search results page
 */
public class IndiaMartSteps {

    private static final Logger log = LogManager.getLogger(IndiaMartSteps.class);

    // ── Shared state (lives for the duration of one scenario) ─────────────────
    ...
    public void displayServiceNames() {
        // Collect names, URLs, and ratings from the search results page
